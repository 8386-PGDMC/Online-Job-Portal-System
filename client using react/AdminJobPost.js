...
function AdminJobPost() {
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:9999/jobpost/categories'); // Adjust the endpoint as needed
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://127.0.0.1:9999/jobpost', formData);
